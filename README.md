...
- **Bottom-aligned unified frame, 60 mm setback:** current user-selected print; both cages share the same floor, two hollow lower box bridges join their bases, and two sloped upper channel ties join their unequal roof heights
...
- `one_piece_rear_cable_friendly_unified_frame_dovetail_ready.stl` / `.3mf` — current user-selected one-piece print: bottom-aligned validated friction cages, two hollow lower bridges, two sloped upper channel ties, and integrated receivers for optional separately printed dovetail gates

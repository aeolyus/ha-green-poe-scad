...
- `green_vent_frame_coupon.stl` / `.3mf` and `splitter_vent_frame_coupon.stl` / `.3mf` — separate 5 mm-deep solid rear perimeter bands from the exact rounded Green and TP-Link cages. At this depth the crop ends before the floor, roof, or side-wall honeycomb openings begin, and both parts are already oriented with their insertion faces on the build plate.
- `vent_frame_coupon.stl` / `.3mf` — the recommended low-material rounded-cage fit test containing both production-derived solid bands as separate objects; approximately 183.9 × 38.24 × 5 mm and 7.4 g of PETG by mesh volume

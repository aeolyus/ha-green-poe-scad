...
The current user-selected first print is **rear cable entry, 60 mm cable-friendly splitter setback, bottom-aligned rounded vent-frame friction cages, two hollow lower box bridges, two sloped upper hollow-box ties, a plain front with no logo, an open LED viewing aperture, and integrated dovetail receivers with the gates optional**. Use `one_piece_rear_cable_friendly_unified_frame_dovetail_ready.3mf` or `.stl`; the separately printed gate plate is not required for the first fit check. The earlier no-upper-tie and no-dovetail files remain available as comparisons.

Smooth continuous tray walls target a light 0.10 mm-per-side friction fit around the physically measured 111.13 mm lower footprint, then follow the measured inward taper beginning 6.35 mm above the bottom and ending at the 107.95 mm top plateau. Low front/rear stops resist sliding without localized ribs, tabs, or clips. The original 3 mm ventilated honeycomb is translated upward so its top is the normal 9.025 mm device seating plane; the open space below replaces the former four riser columns without filling the tray solid. Annular lands preserve the optional factory screw-hole locations. The Green floor, TP-Link floor, and connecting bridge all share this height, and everything remains fused into the mount. Two hollow 8 mm-high box beams now replace the earlier one-sided L-flanges between the cages. Four longitudinal L-section rails continue the TP-Link cage's upper-left, upper-right, lower-left, and lower-right corners into the faceplate. Their horizontal flanges remain flush with the cage floor and roof, while their vertical flanges stay outside the device envelope. This edge-distributed frame resists sag and twist without center bars, raised internal ridges, permanent triangular braces, or cable-path interference. Any desired support beneath the splitter cage can instead be generated and removed by the slicer.
...
The webpage compares three side-by-side rear-entry TP-Link placements, one bottom-aligned unified-frame study, one centered-stacked TP-Link study, three front-Ethernet editions, and one SICSOLINK cable-routing mockup:
...
- **Bottom-aligned unified frame, 60 mm setback:** current user-selected print; both cages share the same floor, two hollow lower box bridges join their bases, and two sloped upper hollow-box ties join their unequal roof heights
...
Status: both rounded-cage device fits are physically validated as snug. The user-selected bottom-aligned unified-frame STL is closed two-manifold geometry and Bambu Studio reports it as manifold; its new upper ties still need this first full physical print. `assembly.stl` remains a multi-body reference file rather than a print target. The optional dovetail gate remains coupon-gated and does not need to be printed for the first fit check.

## Interactive preview

Open `viewer/interactive_viewer.html` through the included static-site layout and choose the splitter, side-by-side or centered-stacked arrangement, rear/front Ethernet entry, applicable spacing or front-jack position, retention, and optional protection. Those decisions map to nine modeled layouts: Compact, Balanced, Cable-friendly, Bottom-aligned unified frame, Centered stacked, Front–center gap, Front–HA right, Front–left, and the angled SICSOLINK cable example. Centered stacked and SICSOLINK are viewer-only studies. You can rotate, zoom, or jump between front, rear, top, perspective, retention-detail, and LED-window views. The hosted copy loads sibling GLBs to keep the initial HTML near 1 MB; `scripts/build.sh html` can regenerate an embedded single-file offline copy. Both forms record selections in the URL hash so a configuration can be bookmarked or shared.

The default view matches the active print: TP-Link, rear Ethernet, 60 mm cable-friendly setback, the bottom-aligned unified cage frame, integrated dovetail receivers with the gates optional, open trays with no chassis, no logo, no shutter hardware, visible simulated LEDs, and auto-rotation off. The comparison controls can still show the alternate supports, logo, captive shutter, front-entry layouts, other retention studies, and ventilated chassis. Those toggles do not change which files are canonical.
...
- `one_piece_rear_cable_friendly_unified_frame_dovetail_ready.stl` / `.3mf` — current user-selected one-piece print: bottom-aligned validated friction cages, two hollow lower bridges, two sloped upper hollow-box ties, and integrated receivers for optional separately printed dovetail gates
- `one_piece_rear_cable_friendly_unified_frame_no_dovetail.stl` / `.3mf` — the same tied cage frame without the optional dovetail receiver geometry
- `one_piece_rear_cable_friendly_vent_frame_no_dovetail.stl` / `.3mf` — earlier one-piece comparison with the physically validated rounded Green and TP-Link cages but no sloped upper ties or dovetail hardware
...
- `viewer/home-assistant-green-rack-unified_roof-no_shutter.glb` — GLB used by the production-default viewer state
...
- The selected `one_piece_rear_cable_friendly_unified_frame_dovetail_ready` files are already faceplate-down. Center the part and do not add a brim. Use nozzle 1 for the PETG model. The lower boxes and upper ties each begin as a roughly 16 mm bridge between already-established cage walls; PETG should normally bridge that distance, but the X2D can use localized PLA support interfaces there if your preview flags them. The support region remains inside nozzle 2's X=20.5–256 mm reach.

...
- `green_vent_frame_coupon.stl` / `.3mf` and `splitter_vent_frame_coupon.stl` / `.3mf` — separate 5 mm-deep solid rear perimeter bands from the exact rounded Green and TP-Link cages. The Green coupon keeps its prior height but adds 0.10 mm clearance on each side; the TP-Link keeps its prior side fit and lowers only the flexible roof by 0.05 mm. At this depth the crop ends before the floor, roof, or side-wall honeycomb openings begin, and both parts are already oriented with their insertion faces on the build plate.
- `vent_frame_coupon.stl` / `.3mf` — the recommended low-material rounded-cage fit test containing both production-derived solid bands as separate objects; approximately 184.1 × 38.24 × 5 mm and 7.4 g of PETG by mesh volume
...
For the preferred rounded vent-frame cage, print `vent_frame_coupon.3mf` first. Its two solid perimeter bands are already oriented with the cropped rear openings on the plate, matching the layer direction of the eventual faceplate-down rack. Slide each device only 5 mm into its matching band. It should confirm the width, taper, bevel, and roof contact without wasting material on ventilation geometry. Because this shorter gauge is more flexible than the complete cage, use it to reject an overly tight fit rather than as a final extraction-force measurement. Do not print the complete rounded-cage rack until both fits pass. Printer calibration, material, and long-term creep affect every retention option, and the completed mount still needs a pull/rattle test before production use.

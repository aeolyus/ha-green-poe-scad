...
- **Skeletal-sleeve alternative:** two rear-loading device-local cages use honeycomb floors and roofs joined by three short posts per side. The roof/floor borders provide the horizontal load paths, so there are no separate side rails and two large bays remain open on each side. They retain the front stops and leave the rear mouths open, following the sparse architecture of the supplied UCG-Fiber 3MF and referenced USW-Lite enclosure.
...
- Skeletal sleeves: 1.2 mm honeycomb roofs, low honeycomb floors, three 6 mm posts per side, zero nominal roof gap, and rear 6 mm/0.5 mm lead-ins. The Green uses four short raised support pads rather than a full-height honeycomb block. Its roof ends at Z=42.225 mm, leaving 0.775 mm inside the nominal 43 mm panel envelope; the TP-Link roof ends at Z=28.0125 mm. Both sleeves omit rear stops for axial loading.
- The cable-friendly skeletal overlay is approximately 57.6 g of PETG by mesh volume, down from approximately 112.9 g for the earlier full-panel sleeve study (about 49% less material before the shared faceplate and bridges).

...
An independent stronger branch was prototyped without copying the licensed mesh: separate Green and TP-Link gates use a shared 60-degree dovetail standard, 2.8 mm rail depth, 2.6 mm neck, 5.8 mm head, 0.25 mm running clearance per flank, and a final 7 mm wedge zone closing to a nominal line fit. A 1.2 mm blind bottom stop and top lead-in avoid thin spring tabs; the exposed top bar remains available for deliberate removal. The receiver towers remain inside the cage's outer wall planes and sit 2.0 mm rearward. Their channels are subtracted through the complete cage/receiver union so the buried rails have a real vertical insertion path. The revised gate keeps those rails straight and replaces the two anti-rattle pads with one smooth lower crossbeam positioned 0.05 mm behind the device's solid lower rear face. A shallow bevel spans the complete beam rather than concentrating force at small tabs, and one clean rear roof opening replaces the two narrow pad slots. The central connector field stays open, but cables must be disconnected while sliding the gate. Discrete cage collision checks at 0, 1, 7, 20, and 40 mm of gate lift are empty for both devices; the installed gates also clear the modeled connector bodies. Standalone gate exports are rotated broad rear face down instead of balancing on a 2.4 mm edge. If pursued, first print the rail coupon with 0.00, 0.05, and 0.10 mm final wedge clearances.
